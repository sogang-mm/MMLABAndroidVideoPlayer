package kr.ac.sogang.mmlab.AndroidVideoPlayer.feature.playback;
...
public class VideoPlaybackService extends Service
{
    ...
    public void loadMedia(@NonNull Uri mediaUri, boolean playWhenReady, long startPosition)
    {
        ...
        if (isLocalFile(mediaUri) && !hasStorageReadPermission())
        {
            //missing permissions
        ...
        if (isLocalFile(mediaUri))
        {
            ...
            if (!hasStorageWritePermission())
            {
                ...
                Logging.logE("Missing local storage permissions for local file!");
                if (isEventListenerValid())
                    eventListener.onMissingPermissions(new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE});
